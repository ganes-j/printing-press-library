...
func SplitRecords(input string, maxBytes int) []Tranche {
	if maxBytes <= 0 {
		maxBytes = 64 * 1024
	}
...
func splitLines(input string, maxBytes int) []Tranche {
	scanner := bufio.NewScanner(strings.NewReader(input))
	scanner.Buffer(make([]byte, 1024), maxBytes*4)
	var out []Tranche
