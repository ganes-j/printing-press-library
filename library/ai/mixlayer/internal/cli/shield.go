...
func newShieldIngestCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tranches := shield.SplitRecords(text, maxBytes)
			...
			return outputJSON(cmd, map[string]any{"tranches": len(tranches), "manifest": manifest, "output": output})
		},
	}
	cmd.Flags().StringVar(&dbPath, "db", "", "SQLite database file path")
...
func newShieldAskCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			redacted, err := shield.Redact(cmd.Context(), s, data, false)
			if err != nil {
				return err
			}
			leaks := shield.Detect(redacted.Text)
			if len(leaks) > 0 && !force {
				return partialFailureErr(fmt.Errorf("masked payload still contains %d detector hits; use --force to send anyway", len(leaks)))
			}
			payload := fmt.Sprintf("Use this masked corpus:\n\n%s\n\nQuestion: %s", redacted.Text, args[0])
			run, err := chatAndSave(cmd.Context(), flags, s, "shield ask", payload, model, 0, showThinking)
			if err != nil {
				return err
			}
			...
			audit := store.AuditRecord{
				...
				PayloadSHA256: sha256Hex([]byte(payload)), ByteCount: len(payload), Model: model, GuardModel: guard,
				MaskedEntities: len(redacted.Entities), LeakedPIICount: len(leaks), CostUSD: run.CostUSD,
			}
	...
	cmd.Flags().StringVar(&guard, "guard", defaultGuardModel, "Guard model recorded in receipts")
